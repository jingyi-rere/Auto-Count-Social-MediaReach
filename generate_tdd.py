...
add_body(doc, (
    ...
    '(reduced from 5 minutes for faster user feedback — the watcher runs sequentially so '
    'each cycle completes fully before sleeping, no overlap between cycles is possible). '
    'Each URL is routed to the best extraction method: '
    'yt-dlp (fast, exact numbers, no browser) for YouTube; '
    'Firefox + DOM for TikTok and X (Twitter); '
...
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
run = p.add_run('— End of Document — Version 3.5 —')
run.italic = True
run.font.size = Pt(9)
...
md_path = '/Users/jjjyyy/Auto-Count Social MediaReach/TDD_Auto_Count_Social_Media_Reach.md'
...
| 3.4 | {today} | jingyi-rere | 3-min interval reason added; 283 tests updated for Instagram arch; latency = 5 URLs ~15-20 min |
...
- Strips hashtags: `re.sub(r'#[\\w\\u00C0-\\u024F\\u4e00-\\u9fff]+', '', caption)`
